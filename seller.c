...
int main(int argc, char *argv[]) {
    ...
    char buffer[256];
    if (argc < 2) {
        fprintf(stderr, "ERROR: please enter port for broadcasting\n");
        exit(EXIT_FAILURE);
    }
    port = atoi(argv[1]);

    char name[255];
    printf("Seller terminal\nPlease enter your name: ");
    scanf("%s", name);
    printf("Welcome %s\n", name);
    ...
    char wantAdTitle[255];
    int wantAdPort;
    printf("Please enter your want ad title: ");
    scanf("%s", wantAdTitle);
    printf("Please enter your want ad port: ");
    scanf("%d", &wantAdPort);

    //printf("%s-%d\n", wantAdTitle, wantAdPort);
    ...
    while (1) {
        ...
        if (FD_ISSET(sockfd, &current_sockets)) { //server has written something for this client
            bzero(buffer, 255);
            int n = read(sockfd, buffer, 255); //read message from server
            if (n < 0)
                error("ERROR: client failed to read buffer from server\n");
    ...
    while (1) {
        ...
        if (FD_ISSET(STDIN, &current_sockets)) {
            ...
            sendto(udp_sockfd, buffer, strlen(buffer), 0, (struct sockaddr *) &bc_adr_sendto,
